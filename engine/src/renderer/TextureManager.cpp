...
void TextureManager::transitionImageLayout(Texture& texture, VkImageLayout oldLayout, VkImageLayout newLayout, VkCommandBuffer commandBuffer, uint32_t mipLevels) {
  ...
  if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
    ...
    barrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;  // Ensures all
                                                           // vkCmdCopyBufferToImage
                                                           // operations complete
    barrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;     // Tells Vulkan that shaders will
                                                           // read this texture

    sourceStage = VK_PIPELINE_STAGE_TRANSFER_BIT;
    destinationStage = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;  // transfer must be
                                                               // done before
                                                               // fragment shader
...
void TextureManager::InitCubemapTexture(Texture& texture, uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage,
  ...
  imageInfo.samples = VK_SAMPLE_COUNT_1_BIT;
  imageInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
  imageInfo.flags = VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT;  // Important for cubemap
...
VkImageView TextureManager::createCubemapImageView(VkImage image, VkFormat format, uint32_t levelCount) {
  VkImageViewCreateInfo viewInfo{};
  viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
  viewInfo.image = image;
...
void TextureManager::transitionCubemapLayout(Texture& texture, VkImageLayout oldLayout, VkImageLayout newLayout, VkCommandBuffer commandBuffer) {
  ...
  if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
    ...
    throw std::invalid_argument("unsupported cubemap layout transition!");
  }
...
TextureManager::Texture TextureManager::createCubemapFromSingleFile(const std::string& filepath, VkFormat format) {
  ...
  if (!pixels) {
    throw std::runtime_error("Failed to load cubemap image: " + filepath);
  }
  ...
  auto copyFace = [&](int faceIndex, int srcX, int srcY) {
    ...
    spdlog::info(
        ...
        "{}, {})",
        faceIndex, srcX, srcY, pixels[testIdx], pixels[testIdx + 1], pixels[testIdx + 2], pixels[testIdx + 3]);
    for (int y = 0; y < faceSize; ++y) {
      for (int x = 0; x < faceSize; ++x) {
        int srcIdx = ((srcY + y) * imgWidth + (srcX + x)) * 4;
        int destIdx = (y * faceSize + x) * 4;
        memcpy(destData + faceIndex * faceDataSize + destIdx, pixels + srcIdx, 4);
      }
    }
  };
  ...
  for (uint32_t face = 0; face < 6; ++face) {
    copyBufferToCubemapFace(texture, stagingBuffer.buffer, commandBuffer, face, face * faceDataSize);
  }
