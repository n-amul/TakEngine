...
void VulkanBase::processInput(float deltaTime) {
  // quick debug
  if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) spdlog::info("W pressed");
  if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) spdlog::info("A pressed");
  if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) spdlog::info("S pressed");
  if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) spdlog::info("D pressed");
  // Movement keys
  ...
  if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS) camera.moveUp(deltaTime);
  if (glfwGetKey(window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS) camera.moveDown(deltaTime);
  ...
  if (action == GLFW_PRESS) {
    switch (key) {
      ...
      case GLFW_KEY_R:
        ...
        app->camera.initialize(glm::vec3(0.0f, 0.0f, 5.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        break;
    }
  }
