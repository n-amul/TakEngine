...
// ------------- file-local helpers (internal linkage TODO:need to include for inheritance) -------------
...
void VulkanBase::drawFrame() {
  // wait until the previous frame has finished then reset the fence
  vkWaitForFences(device, 1, &inFlightFences[currentFrame], VK_TRUE, UINT64_MAX);

  // acquireimage from swap chain
  uint32_t imageIndex;
  ...
  recordCommandBuffer(commandBuffers[currentFrame], imageIndex);  // record the commands we want
  ...
  // Use image-indexed semaphore for signaling
  // This ensures each image has its own semaphore for presentation
...
void VulkanBase::createRenderPass() {
  ...
  // Before subpass 0 starts writing to the color attachment,
  // make sure all previous operations that might have been writing to it (like presentation) are finished.
...
void VulkanBase::recordCommandBuffer(VkCommandBuffer commandBuffer, u32 imageIndex) {
  ...
  // draw command just calls three times for now
...
void VulkanBase::createSyncObjects() {
  // Create semaphores matching swap chain image count
  imageAvailableSemaphores.resize(MAX_FRAMES_IN_FLIGHT);    // For frame-in-flight synchronization
  renderFinishedSemaphores.resize(swapChainImages.size());  // For per-image presentation
...
SwapChainSupportDetails VulkanBase::querySwapChainSupport(VkPhysicalDevice device) {
  ...
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, surface, &details.capabilities);  // init capabilities
...
VkExtent2D VulkanBase::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
  if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
    ...
    // special value in currentExtent to choose the size
    int width, height;
...
void VulkanBase::createSwapChain() {
  ...
  createInfo.clipped = VK_TRUE;              // enabling clipping for performance
  createInfo.oldSwapchain = VK_NULL_HANDLE;  // TODO:null for now, revisit when resize is needed
