...
class TAK_API PBRIBLScene : VulkanBase {
 ...
 private:
  ...
  struct LightSource {
    ...
    glm::vec3 rotation =
        glm::vec3(50.0f, 0.0f, 40.0f);  // For directional light
  } lightSource;
  ...
  std::vector<BufferManager::Buffer>
      shaderMeshDataBuffers;  // One per frame
  // ============= Descriptors =============
  // Descriptor layouts
  struct DescriptorSetLayouts {
    VkDescriptorSetLayout scene{
        VK_NULL_HANDLE};  // matrices + params +skybox
    VkDescriptorSetLayout material{
        VK_NULL_HANDLE};  // per-material textures
    VkDescriptorSetLayout materialBuffer{
        VK_NULL_HANDLE};  // SSBO with all material data
    VkDescriptorSetLayout meshDataBuffer{
        VK_NULL_HANDLE};  // SSBO with all mesh data
  ...
  std::vector<VkDescriptorSet>
      descriptorSetsMeshData;  // One per frame
  ...
  VkPipeline boundPipeline{
      VK_NULL_HANDLE};  // Track current bound pipeline
  ...
  void renderNode(VkCommandBuffer cmdBuffer, tak::Node* node,
                  uint32_t cbIndex,
